...
//        height = w;
    ...
    private void capture() {
        ...
        try {
            ...
            Log.e(TAG , "capture : rotation : " + rotation);
            ...
            if (isCameraFront) {
                ...
                if(rotation == 0 || rotation == 180){
                    //后置
                    mCaptureBuilder.set(CaptureRequest.JPEG_ORIENTATION, ORIENTATION.get(Surface.ROTATION_90));
                }else{
                    mCaptureBuilder.set(CaptureRequest.JPEG_ORIENTATION, ORIENTATION.get(Surface.ROTATION_0));
                }
                ...
                if(rotation == 0 || rotation == 180){
                    //后置
                    mCaptureBuilder.set(CaptureRequest.JPEG_ORIENTATION, ORIENTATION.get(Surface.ROTATION_270));
                }else{
                    mCaptureBuilder.set(CaptureRequest.JPEG_ORIENTATION, ORIENTATION.get(Surface.ROTATION_0));
                }
    ...
    private void configureTransform(int viewWidth, int viewHeight) {
        ...
        if (Surface.ROTATION_90 == rotation || Surface.ROTATION_270 == rotation) {
            ...
            float scale2 = Math.max(
                    mPreviewSize.getWidth() / (float) viewWidth,
                    mPreviewSize.getHeight() / (float) viewHeight
            );
//            float scale3 = Math.max(
//                    mPreviewSize.getWidth() / (float) viewHeight,
//                    mPreviewSize.getHeight() / (float) viewHeight
//            );
            matrix.postScale(scale, scale, centerX, centerY);
            matrix.postScale(scale2, scale2, centerX, centerY);
//            matrix.postScale(scale3, scale3, centerX, centerY);
